    ...
    <>
      ...
      <section className="section" style={{
        ...
        <div className="container-app" style={{ textAlign: 'center' }}>
          ...
          <div className="animate-fade-in-up flex justify-center gap-4 flex-wrap" style={{ animationDelay: '0.4s' }}>
      ...
      {/* ── Find Your Solution ── */}
      <section id="solution" className="section" style={{ background: 'var(--color-surface-2)' }}>
        <div className="container-app">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem', maxWidth: '800px', margin: '0 auto' }}>
            {[
              { role: 'athlete', icon: '🏃', title: 'For Athletes', desc: 'Find the perfect coach tailored just for you.', href: '/get-started?role=client', cta: 'Find a Coach' },
              { role: 'coach', icon: '🎓', title: 'For Coaches', desc: 'Discover all the players you need to complete your dream team.', href: '/get-started?role=coach', cta: 'Find Athletes' },
            ].map(card => (
              <div key={card.role} className="glass" style={{ padding: '2.5rem 2rem', textAlign: 'center', transition: 'transform 0.2s ease, box-shadow 0.2s ease' }}
                onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.transform = 'translateY(-4px)'; (e.currentTarget as HTMLDivElement).style.boxShadow = '0 16px 40px rgba(37,99,235,0.2)'; }}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1.5rem' }}>
            {steps.map((step, i) => (
              <div key={step.num} className="glass" style={{ padding: '2rem', position: 'relative', overflow: 'hidden' }}>
                <div style={{
                  position: 'absolute', top: '-0.5rem', right: '1rem', fontSize: '4rem', fontWeight: 900,
                  color: 'rgba(37,99,235,0.1)', lineHeight: 1, userSelect: 'none'
                }}>{step.num}</div>
      ...
      <section className="section" style={{ background: 'var(--color-surface-2)' }}>
        <div className="container-app">
          <div style={{ textAlign: 'center', marginBottom: '3.5rem' }}>
            ...
            <p style={{ color: 'var(--color-text-muted)', maxWidth: '45ch', margin: '0 auto' }}>Powerful tools built for athletes and coaches who are serious about growth.</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.25rem' }}>
            {features.map(f => (
              <div key={f.title} className="glass" style={{ padding: '1.75rem', display: 'flex', gap: '1rem', alignItems: 'flex-start', transition: 'border-color 0.2s ease' }}
      ...
      <section className="section">
        <div className="container-app">
          <div style={{ textAlign: 'center', marginBottom: '3.5rem' }}>
            ...
            <p style={{ color: 'var(--color-text-muted)', maxWidth: '45ch', margin: '0 auto' }}>Join a platform designed to grow with you.</p>
          </div>
      ...
      {/* ── Waitlist Section ── */}
      <section className="section" style={{ background: 'radial-gradient(ellipse 60% 50% at 50% 50%, rgba(37,99,235,0.12) 0%, transparent 70%), var(--color-surface-2)' }}>
        <div className="container-app" style={{ maxWidth: '600px', textAlign: 'center' }}>
          <h2 style={{ fontSize: 'clamp(1.75rem, 4vw, 2.75rem)', marginBottom: '1rem' }}>
...
function WaitlistForm() {
  return (
    <form
      ...
      method="POST"
      className="glass"
      style={{ padding: '2rem', textAlign: 'left', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}
    >
      <div>
